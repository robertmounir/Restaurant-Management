...
void GUI::PrintMessage(string msg) const	//Prints a message on status bar
{
	...
	pWind->DrawString(10, WindHeight - (int) (StatusBarHeight/1.25), msg); // You may need to change these coordinates later 
	                                                                      // to be able to write multi-line
}
void GUI::printInfo(string msg,int x,float y) const
{
	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD, BY_NAME, "Arial");
	pWind->DrawString(x, WindHeight - (int)(StatusBarHeight /y ), msg);
}
